...
int getMax(Pokemon vetor[], int n) {
	int max = vetor[0].id;
	for (int i = 1; i < n; i++) {
		if (vetor[i].id > max) {
			max = vetor[i].id;
...
// Função de contagem para cada dígito (Counting Sort por dígito)
void countingSort(Pokemon vetor[], int n, int exp) {
	Pokemon output[n]; // Vetor temporário para armazenar resultados
	int count[10] = {0}; // Armazena a contagem de cada dígito
	...
	// Modificar a contagem para obter a posição real de cada dígito
	for (int i = 1; i < 10; i++) {
		count[i] += count[i - 1];
	}
...
void radixSort(Pokemon vetor[], int n) {
	// Encontrar o maior número no vetor para saber o número de dígitos
	int max = getMax(vetor, n);
	
	// Aplicar Counting Sort para cada dígito
	// exp é 10^i onde i é o dígito atual (1s, 10s, 100s, ...)
