...
int getMaxHabilidadeLength(Pokemon vetor[], int n) {
	int maxLen = 0;
	for (int i = 0; i < n; i++) {
		int len = strlen(vetor[i].abilities[0]);
		if (len > maxLen) {
			maxLen = len;
		}
	}
...
// Função de contagem para ordenar com base em uma posição específica da string
void countingSortHabilidade(Pokemon vetor[], int n, int pos) {
	Pokemon output[n]; // Vetor temporário para armazenar o resultado
	int count[256] = {0}; // Armazena a contagem de cada caractere ASCII
	
	// Contar o número de ocorrências de cada caractere na posição `pos`
	for (int i = 0; i < n; i++) {
		int index = (strlen(vetor[i].abilities[0]) > pos) ? (int)vetor[i].abilities[0][pos] : 0;
		count[index]++;
	}
	
	// Modificar a contagem para obter a posição real de cada caractere
	for (int i = 1; i < 256; i++) {
		count[i] += count[i - 1];
	}
...
// Função principal de Radix Sort para strings (ordenando por habilidades)
void radixSort(Pokemon vetor[], int n) {
	// Encontrar o comprimento da habilidade mais longa
	int maxLen = getMaxHabilidadeLength(vetor, n);
	
	// Aplicar Counting Sort para cada caractere, da posição menos significativa à mais significativa
	for (int pos = maxLen - 1; pos >= 0; pos--) {
		countingSortHabilidade(vetor, n, pos);
	}
}
...
int main(void) {
	...
	FILE* log = fopen("860668_radixsort.txt", "w");
	if (log) {
