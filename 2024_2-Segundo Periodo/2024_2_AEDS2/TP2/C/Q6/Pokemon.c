...
void selectionSort(Pokemon* pokemons, int i, int tamanho){
	...
	if (i < tamanho - 1) { 
		...
		selectionSort(pokemons, i + 1, tamanho);
	}
}
...
int main(void) {
	...
	selectionSort(selecionados, 0, i);
	...
	FILE* log = fopen("860668_selecaoRecursiva.txt", "w");
	if (log) {
		fprintf(log, "860668\t%d\t%d\t%.2f\n", comparacoes, movimentacoes, tempoExecucao);
		fclose(log);
	}
