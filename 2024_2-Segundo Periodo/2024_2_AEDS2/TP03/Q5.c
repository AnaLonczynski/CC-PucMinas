...
typedef struct lista{
    char lista[200][500];      //elementos da lista de Strings (pode ser usado para abilities ou types)
...
void preencherVetor(pokemon pokemons[]) {
    ...
    char buffer[1000];
    fgets(buffer, 1000, file);  // Ignora a linha de cabeçalho
    ...
    while (fgets(buffer, 1000, file)) {
        ...
        char novaLinha[1000];
        if (prePartsCount > 2) {
            snprintf(novaLinha, 1000, "%s%s", preParts[0], preParts[2]);
        } else {
            strncpy(novaLinha, buffer, 1000);
        }
...
int main(){
  char entrada[200];
   ...
   int i,j=0,count=0;
    ...
    pokemon *temp = malloc(802*sizeof(pokemon));
